...
class ExchangeGateway(object):
    ...
    ############################################################################
    engin_start_date_time = datetime.utcnow()
    ...
    def get_instmt_snapshot_table_name(cls, exchange, instmt_name):
        ...
        return 'exch_' + exchange.lower() + '_' + instmt_name.lower() + \
               '_snapshot_' + cls.engin_start_date_time.strftime("%Y%m%d%H%M%S")
    ...
    def willStart(self, instmt):
        instmt.set_keep_running(True)
        if self.api_socket:
            self.api_socket.willStart()
    ...
    def stop(self, instmt):
        """
        stop the exchange gateway thread
        """
        instmt.set_keep_running(False)
        if self.api_socket:
            self.api_socket.stop()
        return
    ...
    def insert_order_book(self, instmt):
        ...
        if self.is_local_timestamp:
            instmt.get_l2_depth().date_time = calendar.timegm(datetime.utcnow().timetuple()) # datetime.utcnow().strftime("%Y%m%d %H:%M:%S.%f")
    ...
    def insert_trade(self, instmt, trade):
        ...
        # If local timestamp indicator is on, assign the local timestamp again
        if self.is_local_timestamp:
            trade.date_time = calendar.timegm(datetime.utcnow().timetuple()) # datetime.utcnow().strftime("%Y%m%d %H:%M:%S.%f")
