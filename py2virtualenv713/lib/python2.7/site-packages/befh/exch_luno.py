...
# Store the credentials in your home directory in a file called exch_luno.json, or alternatively change the path_to_creds
...
class ExchGwApiLuno(WebSocketApiClient):
    ...
    def _handle_creds(cls, path_to_creds=None):
        ...
        if not path_to_creds:
            path_to_creds = os.path.join(os.path.abspath(os.path.dirname(__file__)), "exch_luno.json")
    ...
    def parse_l2_depth(cls, instmt, raw):
        ...
        if bids_field in keys and asks_field in keys:
            # Date time
            l2_depth.date_time = calendar.timegm(datetime.utcnow().timetuple()) # datetime.utcnow().strftime("%Y%m%d %H:%M:%S.%f")
    ...
    def parse_trade(cls, instmt, raw):
        ...
        trade.date_time = timestamp # datetime.utcfromtimestamp(timestamp).strftime("%Y%m%d %H:%M:%S.%f")
