...
class WebSocketApiClient(ApiSocket):
    ...
    def connect(self, url,
        ...
        if not self._connecting and not self._connected:
            ...
            self.wst = threading.Thread(target=lambda: self.__start(reconnect_interval=reconnect_interval, ping_interval=ping_interval))
            self.wst.start()
    ...
    def stop(self):
        super(WebSocketApiClient, self).stop()
        if self.ws:
            self.ws.close()

    def __start(self, reconnect_interval=10, ping_interval=None):
