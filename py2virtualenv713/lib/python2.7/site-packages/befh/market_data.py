...
class L2Depth(MarketDataBase):
    ...
    def __init__(self, depth=5):
        ...
        self.date_time = calendar.timegm(datetime(2000, 1, 1, 0, 0, 0).timetuple())# datetime(2000, 1, 1, 0, 0, 0).strftime("%Y%m%d %H:%M:%S.%f")
        self.depth = depth
...
class Trade(MarketDataBase):
    ...
    def types():
        ...
        Return static column types
        """
        return ['int', 'text', 'decimal(10,5)', 'decimal(20,8)', 'int']
...
class Snapshot(MarketDataBase):
    ...
    def values(exchange_name='', instmt_name='', l2_depth=None, last_trade=None, update_type=UpdateType.NONE):
        ...
        Return values in a list
        """
        if l2_depth.is_diff(L2Depth()):
            assert l2_depth.asks[4].price > l2_depth.asks[0].price
            assert l2_depth.bids[0].price > l2_depth.bids[4].price
            assert l2_depth.asks[0].price > l2_depth.bids[0].price, "%f, %f" % (l2_depth.asks[0].price, l2_depth.bids[0].price)
