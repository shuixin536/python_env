...
'''
https://www.coinnest.co.kr/doc/public.html
'''
...
import calendar
class ExchGwApiCoinnest(RESTfulApiSocket):
    """
    ...
    def get_order_book_link(cls, instmt):
        ...
        return "https://api.coinnest.co.kr/api/pub/depth/?coin=%s" % instmt.get_instmt_code()

    @classmethod
    def get_trades_link(cls, instmt):
        if int(instmt.get_exch_trade_id()) > 0:
            # 获取速度太快，可能导致没有新的订单出来，一直获取的是同一条
            return "https://api.coinnest.co.kr/api/pub/trades/?coin=%s&since=%s" % \
                (instmt.get_instmt_code(), instmt.get_exch_trade_id())
        else:
            return "https://api.coinnest.co.kr/api/pub/trades/?coin=%s" % \
    ...
    def parse_l2_depth(cls, instmt, raw):
        ...
        if cls.get_bids_field_name() in keys and \
           cls.get_asks_field_name() in keys:
            ...
            l2_depth.date_time = calendar.timegm(
                datetime.utcnow().timetuple())  # datetime.now().strftime("%Y%m%d %H:%M:%S.%f")
    ...
    def parse_trade(cls, instmt, raw):
        ...
        if cls.get_trades_timestamp_field_name() in keys and \
            ...
            trade.date_time = date_time  # datetime.utcfromtimestamp(date_time).strftime("%Y%m%d %H:%M:%S.%f")

            # Trade side
            trade.trade_side = MarketDataBase.Side.BUY if raw[cls.get_trade_side_field_name()] == 'buy' else 2
...
class ExchGwCoinnest(ExchangeGateway):
    """
    Exchange gateway
    """
    def __init__(self, db_clients):
        ...
        """
        ExchangeGateway.__init__(self, ExchGwApiCoinnest(), db_clients)
        ...
        while instmt.keep_running:
            ...
            for trade in ret:
                ...
                if int(trade.trade_id) > int(instmt.get_exch_trade_id()):
                    #print "exch_trade_id: %s" % trade.trade_id
...
if __name__ == '__main__':
    Logger.init_log()
    exchange_name = 'Coinnest'
    instmt_name = 'LTCBTC'
    instmt_code = 'LTCBTC'
    instmt = Instrument(exchange_name, instmt_name, instmt_code)    
    db_client = SqlClientTemplate()
    exch = ExchGwCoinnest([db_client])
    instmt.set_l2_depth(L2Depth(5))
    instmt.set_prev_l2_depth(L2Depth(5))
    instmt.set_recovered(False)    
